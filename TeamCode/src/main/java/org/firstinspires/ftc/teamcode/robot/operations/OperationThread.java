                ...
                if (this.operationsQueue.size() > 0) {
                    ...
                    if (operation.getOperationIsBeingProcessed()) {
                        if (robot.operationCompleted(operation)) {
                            ...
                            robot.setState("Done-" + operation.getTitle());
...
/*
                        else if (new Date().getTime()/1000%5 == 0){//every 5th second
                            Match.log(title + ": Incomplete operation: " + operation.toString()
                                    + " at " + Match.getInstance().getElapsed()
                                    + " in " + (new Date().getTime() - operation.getStartTime().getTime())
                                    + " mSecs");
                        }
*/
                ...
                if (this.operationsQueue.size() > 0) {
                    Operation operation = this.operationsQueue.get(0);
                    if (!operation.getOperationIsBeingProcessed()) {
                        robot.setState("Start-" + operation.getTitle());
    ...
    public void queueUpOperation(Operation operation) {
        synchronized (threadLock) {
            ...
            //Match.log(title + ": Added operation " + operation.toString());
        }
    }
    ...
    public void abort() {
        synchronized (threadLock) {
            ...
            if (this.operationsQueue.size() > 0) {
                Operation operation = this.operationsQueue.get(0);
                if (operation.getOperationIsBeingProcessed()) {
                    robot.abortOperationCompleted(operation) ;
                }
            }
